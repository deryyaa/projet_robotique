...
class Robot:
    ...
    def capteur_distance(self):
        ...
        # Tant qu'il n'a rien detecté, on fait avancer le capteur dans la direction de robot et on incremente sa distance parcourue
        while not Monde.detecter_collision(capteur_x, capteur_y): 
            print("x et y",capteur_y,capteur_y)
            distanceP_capteur+= 0.5
            capteur_x += direction[0]
            capteur_y += direction[1]
        print(f"Position actuelle du robot : {[self.x, self.y]}, Distance jusqu'à l'obstacle : {distanceP_capteur}")

        if distanceP_capteur<=0.5:
            return True 
        return False
